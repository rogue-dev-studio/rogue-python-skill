...
**Level: max** specialized skill (not merged). Category hint: `language`.

## Summary

Python 3.13 with uv package manager via mise. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `python` or user asks by name
...
2. **Confirm tooling** - Verify `python` runtime/CLI/MCP is available; document install if missing.
